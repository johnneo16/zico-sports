...
export default function ContactSection() {
  ...
  const handleSubmit = (e) => {
    e.preventDefault();                          // ← stops any accidental fire
    const errs = validate();
    ...
    window.open(`https://wa.me/917987461287?text=${encodeURIComponent(lines)}`, '_blank');
    setSent(true);
    setForm(EMPTY);
  };
          ...
          <address className="contact-details">
            {contactInfo.map(({ icon: Icon, label, value, href }) => (
              <div key={label} className="contact-detail">
                <div className="contact-detail__icon" aria-hidden="true"><Icon size={15} /></div>
                <div>
                  <div className="contact-detail__label">{label.toUpperCase()}</div>
                  {href ? (
                    <a href={href} className="contact-detail__value contact-detail__link"
        ...
        <div className="contact-form-wrapper reveal reveal--right reveal--delay-2">
          {sent ? (
            <div className="contact-form__success">
              <div className="contact-form__success-icon"><MessageCircle size={40} aria-hidden="true" /></div>
              <h3 className="contact-form__success-title">Sent via WhatsApp!</h3>
              <p className="contact-form__success-text">Your message is pre-filled in WhatsApp. We'll reply shortly.</p>
            ...
            <form className="contact-form" onSubmit={handleSubmit} noValidate>
              ...
              <div className={`contact-form__field${errors.name ? ' contact-form__field--error' : ''}`}>
                <input type="text" placeholder="Your Name *" value={form.name} autoComplete="name"
                  onChange={e => set('name', e.target.value)} className="contact-form__input"
              ...
              <div className={`contact-form__field${errors.email ? ' contact-form__field--error' : ''}`}>
                <input type="email" placeholder="Email Address" value={form.email} autoComplete="email"
                  onChange={e => set('email', e.target.value)} className="contact-form__input"
              ...
              <div className="contact-form__field">
                <input type="tel" placeholder="Phone / WhatsApp" value={form.phone} autoComplete="tel"
                  onChange={e => set('phone', e.target.value)} className="contact-form__input"
                  aria-label="Phone or WhatsApp" />
              </div>

              {/* Message */}
              <div className={`contact-form__field${errors.message ? ' contact-form__field--error' : ''}`}>
                <textarea placeholder="Your message or gear query... *" rows={4} value={form.message}
                  onChange={e => set('message', e.target.value)} className="contact-form__textarea"
                  aria-label="Message" aria-required="true" />
                {errors.message && <span className="contact-form__error">{errors.message}</span>}
              </div>

              <button type="submit" className="contact-form__submit-btn">
                <MessageCircle size={15} aria-hidden="true" style={{ marginRight: 6 }} />
                SEND VIA WHATSAPP
              </button>
              <p className="contact-form__hint">Opens WhatsApp with your message pre-filled.</p>
            </form>
          )}
        </div>
      </div>
