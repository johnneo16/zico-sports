      ...
      <div className="product-modal animate-scale-in" onClick={(e) => e.stopPropagation()}>
        ...
        >
          <div className="product-modal__hero-overlay">
            ...
            {multi && (
              <>
                <button className="product-modal__img-nav product-modal__img-nav--prev" onClick={(e) => goTo(imgIdx - 1, e)} aria-label="Previous">
                  <ChevronLeft size={16} />
                </button>
                <button className="product-modal__img-nav product-modal__img-nav--next" onClick={(e) => goTo(imgIdx + 1, e)} aria-label="Next">
                  <ChevronRight size={16} />
                </button>
                <div className="product-modal__img-dots" aria-hidden="true">
                  {images.map((_, i) => (
                    <span key={i} className={`product-modal__img-dot${i === imgIdx ? ' product-modal__img-dot--active' : ''}`} onClick={(e) => goTo(i, e)} />
                  ))}
                </div>
              </>
            )}
          ...
          <div className="product-modal__actions">
            <div className="product-modal__pricing">
              ...
              {product.original_price && (
                <>
                  <span className="product-modal__original-price">{formatPrice(product.original_price)}</span>
                  <span className="product-modal__discount">-{discount}%</span>
                </>
              )}
            </div>
            <button className="product-modal__add-btn" onClick={() => { onAddCart(product); onClose(); }}>
              <ShoppingBag size={16} /> ADD TO CART
            </button>
          </div>

          <div className={`product-modal__stock ${product.stock < 10 ? 'product-modal__stock--low' : ''}`}>
            {product.stock < 10
